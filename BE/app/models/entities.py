...
class CoinPurchaseRequest(Base):
    ...
    id: Mapped[int] = mapped_column(ID_TYPE, primary_key=True, autoincrement=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    request_type: Mapped[str] = mapped_column(String(30), default="COIN", nullable=False)
    coin_amount: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    price_krw: Mapped[int] = mapped_column(Integer, nullable=False)
    product_code: Mapped[str | None] = mapped_column(String(50))
...
class ChatRoomMember(Base):
    ...
    __table_args__ = (UniqueConstraint("room_id", "user_id", name="chat_room_members_unique"),)
