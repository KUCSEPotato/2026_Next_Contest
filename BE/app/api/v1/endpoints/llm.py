...
def _build_prompt(conversation_text: str) -> str:
    return (
        ...
        "5. 각 할 일은 반드시 '단계 - 구체적인 작업명 :: 클릭했을 때 볼 상세 내용' 형식으로 작성하세요.\n"
        "6. 단계명은 '기획 단계', '설계 단계', '개발 단계 - 백엔드', '개발 단계 - 프론트엔드', '검증 단계'처럼 묶어서 읽히게 작성하세요.\n"
        "7. 작업명은 '정리하기'보다 '로그인 API 요청/응답 필드 구현하기'처럼 무엇을 구현/작성/검증하는지 드러나게 작성하세요.\n"
        "8. 상세 내용에는 결정해야 할 기준, 구현 범위, 기록해야 할 내용, 확인 방법 중 1~2가지를 구체적으로 쓰세요.\n"
        "9. 프로젝트 완성까지 이어지는 순서를 고려해 기획, 설계, 개발, 검증 단계를 섞어 12~18개를 제안하세요.\n"
        "10. 설명, 마크다운, 코드블록, 추가 문장은 절대 출력하지 마세요.\n"
        "11. 할 일이 없으면 정확히 {\"todos_by_user\": {}} 를 반환하세요.\n\n"

        "출력 예시:\n"
        "{\n"
        '  "todos_by_user": {\n'
        '    "민수": [\n'
        '      "기획 단계 - 프로젝트 주제와 완료 기준 확정하기 :: 팀이 합의한 주제, 핵심 사용자, 완료 기준을 상세 내용에 기록한다.",\n'
        '      "개발 단계 - 백엔드 - 로그인 API 구현하기 :: 이메일 로그인과 GitHub OAuth 중 이번 MVP에 포함할 방식을 정하고 요청/응답 필드를 맞춘다."\n'
        "    ],\n"
        '    "지현": [\n'
        '      "UI 디자인 수정"\n'
        "    ]\n"
