...
def _sync_call_gemini_for_memoir_refine(feelings: str, shortcomings: str) -> str:
    prompt = (
        ...
        "목표는 원문을 예쁘게 고쳐 쓰는 것이 아니라, 프로젝트 정보와 사용자의 성장 기록을 바탕으로 "
        "이 사용자가 무엇을 배우고 느꼈으며 앞으로 어떤 방향으로 발전할 수 있는지 통찰력 있게 정리해주는 것입니다.\n\n"

        "응답 방식:\n"
        "1. 프로젝트 제목, 한줄 소개, 상세 설명, 기술 스택, 분야를 그대로 나열하지 마세요. 그 정보는 해석을 위한 배경으로만 사용하세요.\n"
        "   특히 입력 안의 프로젝트 문장, 서비스 설명, 고유 제목, 기능 목록을 그대로 옮겨 쓰지 마세요.\n"
        "2. 사용자가 쓴 표현을 그대로 반복하거나 템플릿 문장에 끼워 넣지 마세요.\n"
        "3. 입력에 담긴 맥락을 해석해 '왜 이 경험이 의미 있었는지'와 '어떤 역량이 자라고 있는지'를 짚어주세요.\n"
        "4. 아쉬운 점은 비난하지 말고, 다음에 시도할 수 있는 구체적인 개선 방향으로 바꿔주세요.\n"
        "5. 근거 없는 성과나 사용자가 말하지 않은 사실을 지어내지 마세요.\n"
        "6. 너무 감상적이거나 과장된 문체는 피하고, 따뜻하지만 담백한 말투로 작성하세요.\n"
        "7. 전체 분량은 450~650자 정도로 제한하세요.\n"
        "8. 2~3개의 짧은 문단으로 작성하세요.\n"
        "9. 마지막 문단은 '다음엔 ... 어떨까요?'처럼 AI의 개인화된 제안 한마디로 마무리하세요. 사용자의 기술/분야/아쉬움을 바탕으로 다음에 해볼 만한 프로젝트 유형과 성장 포인트를 함께 제안하세요.\n"
        "10. 제목, JSON, 마크다운, 따옴표, 불릿 목록은 쓰지 마세요.\n\n"
